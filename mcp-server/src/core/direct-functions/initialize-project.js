...
export async function initializeProjectDirect(args, log, context = {}) {
	...
	try {
		...
		resultData = {
			...
			next_step:
				'Now that the project is initialized, the next step is to create the tasks by parsing a PRD. This will create the tasks folder and the initial task files (tasks folder will be created when parse-prd is run). The parse-prd tool will require a prd.md file as input (typically found in .taskmaster/docs/ directory). You can create a prd.md file by asking the user about their idea, and then using the .taskmaster/templates/example_prd.md file as a template to generate a prd.md file in .taskmaster/docs/. You may skip all of this if the user already has a prd.md file. You can THEN use the parse-prd tool to create the tasks. So: step 1 after initialization is to create a prd.md file in .taskmaster/docs/prd.md or confirm the user already has one. Step 2 is to use the parse-prd tool to create the tasks. Do not bother looking for tasks after initialization, just use the parse-prd tool to create the tasks after creating a prd.md from which to parse the tasks. You do NOT need to reinitialize the project to parse-prd.',
			...result
		};
		success = true;
		log.info(
